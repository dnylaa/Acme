            ...
            <div class="d-flex justify-content-between align-items-center mb-3">
                ...
                <form action="{{ route('admin.informations.index') }}" method="GET"
                    ...
                    @if (request('search') || request('status') !== null)
                        <a href="{{ route('admin.informations.index') }}" class="btn btn-sm btn-outline-danger ms-2">
                            Reset
                        </a>
                    @endif
                </form>

                <a href="{{ route('admin.informations.create') }}" class="btn btn-sm btn-primary px-3">
                        ...
                        <tbody>
                            @foreach ($information as $val)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ route('admin.informations.show', $val->slug) }}"
                                    ...
                                    <td class="text-center">
                                        ...
                                        <a href="{{ route('admin.informations.edit', $val->slug) }}"
                                                class="btn btn-sm btn-success mx-1" title="Edit">
                                                <i class="fas fa-pencil"></i>
                                            </a>
                                            <form action="{{ route('admin.informations.destroy', $val->slug) }}" method="POST"
                                                class="d-inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-sm btn-danger my-1" title="Hapus"
                                                    onclick="return confirm('Apakah Anda yakin ingin menghapus  ini? Tindakan ini tidak dapat dibatalkan.')">
